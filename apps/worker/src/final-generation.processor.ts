...
export async function startFinalGenerationWorker() {
  ...
  const worker = new Worker(QUEUE_NAME, async (job: Job) => {
    ...
    console.log(`Generating final document: ${documentId} (Format: ${outputFormat})`);
    ...
    try {
      ...
      if (outputFormat === OutputFormat.PDF) {
        console.log(`Converting final document ${documentId} to PDF...`);
        try {
      ...
      // 6. Save final document to S3
      const documentKey = `documents/${documentId}.${extension}`;
